...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (token) {
    ...
    async function obtenerPublicacionesPorCategoria(categoria) {
      try {
        const response = await fetch(
          `http://localhost:3000/publicacion/misPublicaciones/categoria/${categoria}`,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
          throw new Error(
            `Error al obtener las publicaciones: ${response.status} ${response.statusText}`
          );
        }
    ...
    async function obtenerMisPublicaciones() {
      ...
      try {
        const response = await fetch(
          "http://localhost:3000/publicacion/misPublicaciones",
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
